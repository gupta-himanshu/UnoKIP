...
trait TopTrends {
  val dbAnalysisServices: AnalysisDBServices

  def trending1(newTweets: RDD[Tweet]): Future[List[Trend]] = {

    val oldTrends = dbAnalysisServices.findTrends()
    val topTrends = for {
    ...
    oldTrends.onComplete {
      case Success(x) =>
        dbAnalysisServices.removeTrends()
      case Failure(err) =>
    }
    topTrends.map { x => x.map { x => dbAnalysisServices.insertTrends(x) } }
