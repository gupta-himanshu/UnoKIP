...
testM.controller("MyController",function($scope, $log, $filter) {
					 ...
					 * scalaDaysSessionData.Name.split("~"); var twitterId =
					 * scalaDaysSessionData.TwitterID.split("~"); $scope.myData =
					 * []; for(i=0;i< names.length;i++){ $scope.myData.push({
					 * "image": images[i], "name" : names[i], "twitterId" :
					 * twitterId[i] }) }
					 * 
